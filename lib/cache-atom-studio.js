...
export default {
		...
		this.restApi({
			...
			success: function(response, textStatus, jqXHR){
				if (response.status != 1) {
					...
					_self.updateFiles(response.directory, response.files);
	...
	delete(path) {
		...
		let ex = filename.split(".")[filename.split(".").length-1].toLowerCase();
		if ((ex != 'cls') && (ex != 'inc') && (ex != 'mac') && (ex != 'int')) {
			return;
		}
	...
	updateFiles(rootPath, files) {
		for(i in files) {
			...
			if (this.isExistFile(filepath) == false) {
