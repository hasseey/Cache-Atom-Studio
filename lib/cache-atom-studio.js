	...
	activate(state) {
		...
		this.subscriptions.add(
			atom.commands.add('atom-workspace', {
				...
				'cache-atom-studio:download': () => this.download(),
				'cache-atom-studio:download-all': () => this.downloadAll(),
				'cache-atom-studio:export': () => this.export(),
				'cache-atom-studio:export-select': () => this.exportSelect(),
		...
		this.restApi({
			...
			success: function(response, textStatus, jqXHR){
				...
				if (response.status != 1) {
					...
					_self.saveExportFile(exportPath, response.files);
					atom.notifications.addSuccess("Successful export of file.");
				}
			},
			error: function(jqXHR, textStatus, errorThrown){
				ccnt.endAjax();
