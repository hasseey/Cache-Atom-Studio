...
export default {
	...
	activate(state) {
		...
		this.consolePanel = atom.workspace.getBottomDock();
		console.log(this.consolePanel);
		/*
		this.consolePanel = atom.workspace.addBottomPanel({
			item:this.CacheAtomStudioConsole.getElement(),
			visible: false,
			priority: 1
		});
		*/
	...
	update(path) {
		...
		if (textSource == "") {
			let paths = [path];
			console.log(paths);
			let tb = new File(paths, false);
			console.log(tb);
			textSource = tb.read(false);
		...
		this.restApi({
			...
			success: function(response, textStatus, jqXHR){
				...
				if (response.status != 1) {
					...
					if (!_self.modalPanel.isVisible()) {
						_self.consolePanel.show();
					}
					_self.CacheAtomStudioConsole.writeLine(response.log);
				}
			},
